...
tcf=0:0.01:20;
x1cf=0.02368; x2cf=0.02368;            %%%%posiciones iniciales
...
% axis([-0.06 0.04 -0.06 0.045])

subplot(2,1,1)
plot(t,x1,'r',r,z,'b',tcf,xt1,'g','LineWidth',2)
title('$\textrm{Case 2.}\ m_1=0.12723\ kg, m_2=0.20306\ kg,\ k_1=10.57372949\ N/m,k_2=9.792093254\ N/m$','Interpreter','latex')
xlabel('$t\ [seg]$','Interpreter','latex')
ylabel('$x_{1}(t)\ [mts]$','Interpreter','latex')
legend({'\textrm{Data}\ $x_1$','$x_1(t)\ \textrm{Caputo}$','$x_1(t)\ \textrm{C-Fabrizio}$'},'Interpreter','latex','Location','northeast','FontSize',14)
axis([0 20 -0.045 0.035])
subplot(2,1,2)
plot(t,x2,'r',r,w,'b',tcf,xt2,'g','LineWidth',2)
legend({'\textrm{Data}\ $x_2$','$x_2(t)\ \textrm{Caputo}$','$x_1(t)\ \textrm{C-Fabrizio}$'},'Interpreter','latex','Location','northeast','FontSize',14)
xlabel('$t\ [seg]$','Interpreter','latex')
ylabel('$x_{2}(t)\ [mts]$','Interpreter','latex')
axis([0 20 -0.06 0.04])

%subplot(3,1,3)
%plot(t,x1,'g',t,x2,'yellow',r,z,'b',r,w,'r','LineWidth',1)
%legend({'Datax1','Datax2','x1(t)','x2(t)'},'Location','northeast','FontSize',14)
...
% xlabel('$t\ [seg]$','Interpreter','latex')
% ylabel('$x_{1}(t)\ [mts]$','Interpreter','latex')
% line([2 cm1+2],[0.025 0.025],'color','m','linewidth',2)
% text(2+cm1+0.1,0.025,'dFD length')
% for i=1:length(cSq1)
...
% xlabel('$t\ [seg]$','Interpreter','latex')
% ylabel('$x_{2}(t)\ [mts]$','Interpreter','latex')
% line([2 cm2+2],[0.035 0.035],'color','m','linewidth',2)
% text(2+cm2+0.1,0.035,'dFD length')
% for i=1:length(cSq2)
...
% xlabel('$t\ [seg]$','Interpreter','latex')
% ylabel('$x_{1}(t)\ [mts]$','Interpreter','latex')
% line([2 cm3+2],[0.025 0.025],'color','m','linewidth',2)
% text(2+cm3+0.1,0.025,'dFD length')
% for i=1:length(cSq3)
...
% xlabel('$t\ [seg]$','Interpreter','latex')
% ylabel('$x_{2}(t)\ [mts]$','Interpreter','latex')
% line([2 cm4+2],[0.035 0.035],'color','m','linewidth',2)
% text(2+cm4+0.1,0.035,'dFD length')
% for i=1:length(cSq4)
